...
class Database
{
   ...
   return false;
    ...
    public function updateBlog($header,$category,$photo,$text,$link,$blogid){
        $stmt = $this->getConnection()
        ->prepare("UPDATE blog  SET header='{$header}',category='{$category}',photo='{$photo}',text='{$text}',link='{$link}'  WHERE blog_id= :id");
        $stmt->execute(['id' => $blogid]);
    }
    public function deleteBlog($blogid){
        $stmt = $this->getConnection()
        ->prepare("DELETE   from blog   WHERE blog_id= :id");
        $stmt->execute(['id' => $blogid]);
    }
